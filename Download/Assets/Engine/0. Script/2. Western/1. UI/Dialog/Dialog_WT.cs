...
public class Dialog_WT : Dialog<DialogData_WT>
{
    [Header("GameObject")]
    ...
    private void Update_Dialog()
    {
        ...
        else if (!m_isTyping)
        {
            ...
            if (m_dialogIndex < m_dialogs.Length)
            {
                ...
                {
                    case DialogData_WT.DIALOGEVENT_TYPE.DET_NONE:
                        Update_None();
                        break;

                    case DialogData_WT.DIALOGEVENT_TYPE.DET_FADEIN:
                        Update_FadeIn();
                        break;

                    case DialogData_WT.DIALOGEVENT_TYPE.DET_FADEOUT:
                        Update_FadeOut();
                        break;

                    case DialogData_WT.DIALOGEVENT_TYPE.DET_FADEOUTIN:
                        Update_FadeOutIn();
                        break;

                    case DialogData_WT.DIALOGEVENT_TYPE.DET_NEXTMAIN:
                        Update_NextMain();
                        break;
                }
            }
    ...
    private void Update_FadeOut()
    {
        if (!string.IsNullOrEmpty(m_dialogs[m_dialogIndex].nextInfo))
        {
            UIManager.Instance.Start_FadeOut(1f, Color.black,
                () => Start_Dialog(GameManager.Instance.Load_JsonData<DialogData_WT>(m_dialogs[m_dialogIndex].nextInfo)), 0f, false);
        }
        else
        {
            // 비어있을 시 페이드 아웃만 진행
    ...
    #region Common
    public void Start_Dialog(DialogData_WT[] dialogs = null)
    {
        m_dialogs = dialogs;
