...
namespace Horror
{
    public class WeaponManagement<T> where T : class
    {
        ...
        public WeaponManagement(HorrorPlayer owner)
        {
            m_player = owner;
        }
        ...
        public void Add_Weapon(NoteItem noteItem)
        {
            ...
            if (m_curWeapon == -1)
            {
                m_player.gameObject.transform.GetChild(0).GetChild(0).GetChild(1).GetChild(1).GetChild(1).gameObject.SetActive(true); // 손 활성화
        ...
        {
            if (weaponIndex < 0 || weaponIndex >= m_weapons.Count || m_player.StateMachine.CurState == (int)HorrorPlayer.State.ST_CHANGE) 
                return;
